...
class Segment:
    ...
    def intersect(self, segment):
        ...
        if (horizontal.start.x <= vertical.start.x and vertical.start.x <= horizontal.start.x + horizontal.offset.x
            and vertical.start.y <= horizontal.start.y and horizontal.start.y <= vertical.start.y + vertical.offset.y):
            return Vector(vertical.start.x, horizontal.start.y)
...
def parseWire(input):
    ...
    for instruction in input.split(','):
        ...
        start = Vector(min(x, toX), min(y, toY))
        segments.append(Segment(start, Vector(abs(toX-x), abs(toY-y))))
...
closestIntersection = Vector(9999999,9999999)

for segment1 in wire1:
    for segment2 in wire2:
        intersection = segment1.intersect(segment2)
        if intersection != False and intersection.manhattanLength() < closestIntersection.manhattanLength():
            closestIntersection = intersection

print(closestIntersection)
print(closestIntersection.manhattanLength())
