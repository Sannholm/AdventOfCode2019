...
wire1Input, wire2Input = INPUT.split('\n')
...
class Segment:
    ...
    def isParellelTo(self, segment):
        return self.isHorizontal() == segment.isHorizontal()

    def intersect(self, segment):
        if self.isParellelTo(segment):
            return False

        horizontal = self if self.isHorizontal() else segment
        vertical = self if not self.isHorizontal() else segment

        horizontalStart = min(horizontal.start.x, horizontal.start.x + horizontal.offset.x)
        horizontalEnd = max(horizontal.start.x, horizontal.start.x + horizontal.offset.x)
        verticalStart = min(vertical.start.y, vertical.start.y + vertical.offset.y)
        verticalEnd = max(vertical.start.y, vertical.start.y + vertical.offset.y)

        if (horizontalStart <= vertical.start.x and vertical.start.x <= horizontalEnd
            and verticalStart <= horizontal.start.y and horizontal.start.y <= verticalEnd):
            return Vector(abs(vertical.start.x - horizontal.start.x), abs(horizontal.start.y - vertical.start.y))

        return False

    def __repr__(self):
        return f'({self.start}+{self.offset})'
...
def parseWire(input):
    ...
    for instruction in input.split(','):
        ...
        start = Vector(x, y)
        segments.append(Segment(start, Vector(toX-x, toY-y)))
...
def cost(wire, intersectionIndex):
    if intersectionIndex < 0:
        return 0

    total = 0
    for segment in wire[0:intersectionIndex+1]:
        total += abs(segment.offset.x) + abs(segment.offset.y)
    return total

wire1 = parseWire(wire1Input)
wire2 = parseWire(wire2Input)
print(wire1)
print(wire2)

cheapestIntersection = 9999999999

for segmentIndex1 in range(1, len(wire1)):
    for segmentIndex2 in range(1, len(wire2)):
        intersection = wire1[segmentIndex1].intersect(wire2[segmentIndex2])
        if intersection != False:
            totalCost = cost(wire1, segmentIndex1-1) + cost(wire2, segmentIndex2-1)
            totalCost += intersection.x + intersection.y
            print(f'{wire1[segmentIndex1]}x{wire2[segmentIndex2]} at {intersection}, cost: {totalCost}')
            if totalCost < cheapestIntersection:
                cheapestIntersection = totalCost

print(cheapestIntersection)
